...
def check_nla_invariant(invariant, name, loop_idx=1, ctx_simplify=False):
    ...
    if name == 'cohencu':
        ...
        res_in, model = z3_checks.cohencu_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()

        if res_out== z3.unknown:
            print('Check timout! (Invariant probably incorrectut)')
        return name, res_out, invariant, doc_invariant, res_in, model
    ...
    if name == 'cohendiv':
        ...
        s.add(Not(Implies(invariant, doc_invariant)))
        if loop_idx == 2:
            res_in, model = z3_checks.cohendiv_z3.full_check(z3_vars, invariant, loop_index=2)
        elif loop_idx != 2:
            res_in, model = z3_checks.cohendiv_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'dijkstra':
        ...
        s.add(Not(Implies(invariant, doc_invariant)))
        if loop_idx == 2:
            res_in, model = z3_checks.dijkstra_z3.full_check(z3_vars, invariant, loop_index=2)
        elif loop_idx != 2:
            res_in, model = z3_checks.dijkstra_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'divbin':
        ...
        s.add(Not(Implies(invariant, doc_invariant)))
        if loop_idx == 2:
            res_in, model = z3_checks.divbin_z3.full_check(z3_vars, invariant, loop_index=2)
        elif loop_idx != 2:
            res_in, model = z3_checks.divbin_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'egcd':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.egcd_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'egcd2':
        ...
        s.add(Not(Implies(invariant, doc_invariant)))
        if loop_idx == 2:
            res_in, model = z3_checks.egcd_z3.full_check(z3_vars, invariant, loop_index=2)
        elif loop_idx != 2:
            res_in, model = z3_checks.egcd_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'egcd3':
        ...
        if loop_idx == 2:
            ...
            res_in, model = z3_checks.egcd_z3.full_check(z3_vars, invariant, loop_index=3)
        else:
            res_in, model = z3_checks.egcd_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'fermat2':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.fermat2_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'freire1':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.freire1_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'freire2':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.freire2_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'geo1':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.geo1_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'geo2':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.geo2_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'geo3':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.geo3_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'hard':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.hard_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'lcm1':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.lcm1_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'lcm2':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.lcm2_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'mannadiv':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.mannadiv_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'prod4br':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.prod4br_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'prodbin':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.prodbin_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'ps2':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.ps2_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'ps3':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.ps3_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'ps4':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.ps4_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'ps5':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.ps5_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'ps6':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.ps6_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    if name == 'sqrt1':
        ...
        invariant = z3.simplify(z3.And(invariant))
        s.add(Not(Implies(invariant, doc_invariant)))
        res_in, model = z3_checks.sqrt1_z3.full_check(z3_vars, invariant, loop_index=1)
        res_out = s.check()
    ...
    #if res_out== z3.unknown:
    #    print('Check timout! (Invariant probably incorrectut)')
    #return res_out, invariant, doc_invariant, res_in
