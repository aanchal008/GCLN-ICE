...
def infer_numinv_bounds(problem_name, loop_index=1, progbar=True, csv_path='../benchmarks/code2inv/traces/',
        weight_reg=True, max_ineq_deg=1, ineq=0, partial=[]):

    learned_eqs = [And(True)]
    print("Partial is following:", partial)
    if len(partial) >= 3:
        #print(partial)
        learned_eqs, _ = compose_invariant(partial[0], partial[1], partial[2], [], problem_name)

    #if type(ineq) == list:
    df = pd.read_csv('../benchmarks/code2inv/traces/' + problem_name, skipinitialspace=True)
    ...
    """
        ...
        if ineq == -1:
            return []
        assert ineq == 0
        df = pd.read_csv("../benchmarks/code2inv/traces/" + str(problem_name) + ".csv", skipinitialspace=True)
        #df = df[df.trace_idx==loop_index]
