...
class config:
    ...
    def run(self, random_seed=True):
        ...
        print("inequality bounds are:", ineq_bounds)
        if self.res is None:
            self.res = [[] for i in range(5)]
        
        print("result learned is following:", self.res)
        print("res[0]", self.res[0])
        print("res[1]", self.res[1])
        print("res[2]", self.res[2])
        print("\n") 
        full_res = list(self.res) + [ineq_bounds]
        print("full result is following:\n", full_res)
        return full_res
    ...
    for p in pnames:
        ...
        # for i in [1]: 
            ...
            for test in range(ntests):
                ...
                total_tests += 1
                invariant, z3_vars = compose_invariant(res[0], res[1], res[2], res[3], p)
                name, check_res, full_invariant, doc_invariant, result_in, model= check_nla_invariant(invariant, p, loop_idx, ctx_simplify=True)
                datastring = str(name) + "," + str(check_res) + ", " + str(full_invariant) + ", " + str(doc_invariant) + "," + str(result_in) + "," + str(model) + "\n"
                #print("datastring is:",datastring)

                #field_names = ['check_res', 'full_invariant', 'doc_invariant', 'result', 'model']

                #with open('result.csv', 'a') as f_object:
                #    dict_writer = DictWriter(f_object, fieldnames = field_names)
                #    dict_writer.writerow(datastring)
                #    f_object.close()
